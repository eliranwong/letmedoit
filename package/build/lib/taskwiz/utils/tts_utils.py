...
class TTSUtil:
    ...
    def play(content, language=""):
        if config.tts:
            try:
                credentials_GoogleCloudTextToSpeech = os.path.join(config.taskWizAIFolder, "credentials_GoogleCloudTextToSpeech.json")
                ...
                if os.path.isfile(credentials_GoogleCloudTextToSpeech):
                    ...
                    audioFile = os.path.join(config.taskWizAIFolder, "temp", "gctts.mp3")
                    if not language:
                    ...
                    audioFile = os.path.join(config.taskWizAIFolder, "temp", "gtts.mp3")
    ...
    @staticmethod
    def getGttsFilename():
        folder = os.path.join(config.taskWizAIFolder, "temp")
